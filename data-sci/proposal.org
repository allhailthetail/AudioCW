...
#+SUBTITLE: Exploratory Data Analysis of Morse Code Signals in R
...
Morse Code is transmitted as short and long tones /(dits and dahs)/ separated by precisely-timed periods of silence.
...
* Planned Scope
** Goals
The primary focus of the project, at least for the duration of the course, will be on conducting EDA and data visualization. As *stretch goals,*
It would be beneficial to the ~AudioCW project~ [cite:@audioCW] as a whole to continue exploring Machine Learning and using the data obtained from analyzing the data for
training the model. As such, the primary goals are as follow:

1. Apply basic pre-processing to Morse Code audio recordings (denoising, filtering)
2. Generate spectrograms and visualize audio patterns
3. Compute acoustic indices such as Acoustic Complexity Index (ACI) and Acoustic Richness (AR)
4. Extract simple features, such as average tone duration, for estimating the Words Per Minute of the transmission.
5. Create clear visualizations of the underlying data of interest in each file.
6. Generate a dataset that would be the basis for a future Machine Learning project.
...
The proposed approach is to use computer-generated training data originally intended for a /Morse Learning Machine Challenge/ from Kaggle [cite:@ag1le2014],
which will be analyzed in R using various candidate libraries to conduct *Exploratory Data Analysis* using the following R libraries:
  - Explore the potential to apply bioacoustic analysis more broadly to digital signal processing using ~Sonic Screwdriver~ [cite:@ssdriverCran]
  - Calculate the Acoustic Indices using ~soundecology~ [cite:@soundecology]
  - Conduct basic sound analysis with ~Seewave~  [cite:@seewaveCran]
  - Utilize ~R~ Programming Language [cite:@RLang], ~GNU Emacs~ [cite:@emacs], and ~GNU Org Mode~ [cite:@orgmode]
    as the main programming development environment.

*** Proposed Visualization Techniques:
  1. Full-color spectrograms and waveforms of relative amplitude envelope, annotated tone on/off segments.
...
** Machine Learning
In the unlikely case that there is time permitting to explore the *flex goals,* this research could extend into Machine Learning and interact with other software in the parent project.
...
* Desired Outcomes
To aid in the visual evaluation, openly-available audio software, such as Tenacity [cite:@tenacityFOSS], can be used to visually and audibly compare our generated
...
/Levenshtein distance/, by which the challenge author claims the results can be objectively evaluated.
